...
- **`CommonEventData` struct:** Contains fields present in every hook event:
  ...
  - `cwd: String`
  - `permission_mode: PermissionMode` - Current permission mode: `Default`, `Plan`, `AcceptEdits`, `Auto`, or `BypassPermissions`
- **`PermissionMode` enum:** Indicates the current permission level for the session:
  ...
  - `Auto` - Claude works autonomously within configured guardrails
